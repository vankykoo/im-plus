...
- **容灾恢复机制**: 启动时可从数据库恢复最大序列号，防止Redis缓存失效导致序列号不连续。
...
- ✅ **心跳保活与连接高可用**: 客户端内置连接存活检测器(Liveness Checker)，能主动发现并关闭"假死"连接，并触发健壮的自动重连逻辑，确保弱网环境下的高可用。
- ✅ **消息存储**: 统一消息表设计，支持私聊和群聊消息统一管理
- ✅ **离线消息**: 完整的离线消息拉取和会话同步机制，支持断点续传
- ✅ **消息已读回执**: 实现端到端的已读状态追踪。私聊精确到每条消息，群聊采用Redis计数器模式并对大群进行性能优化。
- ✅ **超时重发**: 基于时间轮算法的消息超时重发机制，指数退避策略
- ✅ **会话管理**: 会话列表同步，未读数管理，最新消息预览
- ✅ **分布式架构**: 微服务架构，支持水平扩展，无状态设计
- ✅ **高性能序列号**: 独立序列号生成服务，Redis+Lua脚本原子操作，分段预分配策略
- ✅ **服务治理**: 基于Nacos和OpenFeign实现完整的服务注册、发现、配置管理和声明式调用。
- ✅ **消息已读回执**: 不仅有送达ACK，更有完整的已读回执功能，支持私聊和群聊的差异化实现。
- ⚠️ **用户管理**: 好友关系等功能待完善。
### 🔥 技术特性
- 🌐 **微服务治理**: 基于Nacos的服务注册与发现、配置中心，以及基于OpenFeign的声明式服务调用和负载均衡。
...
- **数据库**: MySQL 8.0.30 + MyBatis Plus 3.5.10.1 - 关系型数据库持久化
...
- **ACK消息Topic**: TOPIC_MESSAGE_ACK，使用`MessageListenerConcurrently`并发处理，最大化确认消息的处理效率，避免阻塞业务消息。
- **消费者精细化策略**: 针对业务消息和ACK消息采用不同的消费模型（顺序消费 vs 并发消费），在保证消息顺序性的同时兼顾了系统吞吐量。
...
|------------|---------|-----|----------|------|
| `im:session:user:{userId}` | String | 30分钟 | 存储用户会话信息（UserSession对象），包括网关节点、连接状态等 | `im:session:user:12345` |
| `im:online:users` | Set | 无 | 维护当前在线用户集合，用于快速在线状态查询 | `im:online:users` |
| `user:status:{userId}` | String | 5分钟 | 用户状态缓存，值为UserStatusInfo对象 | `user:status:12345` |
| `friendship:{userId1}:{userId2}` | String | 10分钟 | 好友关系缓存，值为FriendshipInfo对象 | `friendship:123:456` |
#### 2. 消息缓存相关键
| Redis键模式 | 数据类型 | TTL | 用途说明 | 示例 |
|------------|---------|-----|----------|------|
| `msg:{msgId}` | String | 1天 | 缓存消息完整内容JSON，减少数据库查询 | `msg:1234567890` |
| `user:msg:list:{userId}` | ZSet | 无 | 用户消息列表，score为seq，value为msgId，自动维护最近1000条 | `user:msg:list:12345` |
| `conversation:latest:{conversationId}` | String | 30天 | 会话最新消息信息JSON，用于会话列表显示 | `conversation:latest:private_123_456` |
#### 3. 服务端序列号生成相关键
| Redis键模式 | 数据类型 | TTL | 用途说明 | 示例 |
|------------|---------|-----|----------|------|
...
| Redis键模式 | 数据类型 | TTL | 用途说明 | 示例 |
|------------|---------|-----|----------|------|
| `user:last:read:seq:{userId}:{conversationId}` | String | 30天 | 用户在特定会话中的已读序列号 | `user:last:read:seq:123:private_123_456` |
| `group:read:count:{msgId}` | String | 7天 | 群聊消息已读人数计数器 | `group:read:count:1234567890` |
| `group:read:users:{msgId}` | Set | 7天 | 群聊消息已读用户列表（小群组使用） | `group:read:users:1234567890` |
| `user:conversation:seq:{userId}` | Hash | 30天 | 用户各会话的同步序列号 | `user:conversation:seq:12345` |
| `im:message:idempotent:{clientSeq}` | String | 5分钟 | 消息幂等性记录，防止重复处理 | `im:message:idempotent:client_123_001` |
#### 5. 会话管理相关键
| Redis键模式 | 数据类型 | TTL | 用途说明 | 示例 |
|------------|---------|-----|----------|------|
| `user:conversation:list:{userId}` | ZSet | 无 | 用户会话列表，score为时间戳，按时间排序 | `user:conversation:list:12345` |
| `user:conversation:unread:{userId}:{conversationId}` | String | 30天 | 用户各会话未读消息计数 | `user:conversation:unread:12345:private_123_456` |
#### 6. 群组成员管理相关键
| Redis键模式 | 数据类型 | TTL | 用途说明 | 示例 |
|------------|---------|-----|----------|------|
| `group:members:{groupId}` | Set | 24小时 | 群组成员列表缓存 | `group:members:group_789` |

#### 7. 客户端存储相关键
| Redis键模式                                                 | 数据类型 | TTL | 用途说明                | 示例                                                 |
|----------------------------------------------------------|---------|-----|---------------------|----------------------------------------------------|
| `client:sync:seq:{userId}`                               | String | 30天 | 客户端用户级全局序列号         | `client:sync:seq:12345`                            |
| `client:sync:conversation_seq:{userId}:{conversationId}` | String | 7天 | 客户端会话级序列号          | `client:sync:conversation_seq:123:private_123_456` |
## 🔧 技术债务与改进建议

本部分旨在客观评估项目当前存在的技术债务，并提出具体的、可操作的改进建议，以指导未来的迭代方向。
...
1.  **硬编码的配置值**
    *   **问题**: 项目中仍存在大量硬编码的"魔术数字"，尤其是在核心业务逻辑中。例如 `GatewayPushMessageConsumer` 中的线程池大小、`MessageTimeoutManager` 中的时间轮参数（tick duration, wheel size）、`ConversationWorkerPool` 中的队列容量和超时时间等。
    *   **影响**:
        *   **维护困难**: 修改这些关键参数需要重新编译和部署服务，降低了运维效率。
        *   **风险较高**: 在代码中直接修改参数容易出错，且无法针对不同环境（开发、测试、生产）进行灵活配置。
    *   **解决方案**:
        *   **配置类**: 为每个模块的核心组件（如超时管理器、会话工作池）创建对应的 `@ConfigurationProperties` 类，将所有相关参数（如线程数、队列大小、超时时间、开关等）集中管理。
        *   **外部化配置**: 将这些参数移至 `application.yml` 或 Nacos 配置中心，实现动态调整和环境隔离。

2.  **部分异常处理过于宽泛**
    *   **问题**: 在一些远程调用和服务逻辑中，存在 `catch (Exception e)` 这样的通用异常捕获块。这掩盖了问题的具体类型（例如，是网络超时、序列化失败还是业务逻辑错误）。
    *   **影响**:
        *   **问题定位困难**: 无法快速区分是瞬时网络问题还是持续的服务端 bug，增加了故障排查时间。
        *   **降级策略不精确**: 无法针对不同类型的异常执行最合适的降级或重试策略。例如，对于参数校验失败的业务异常，不应该重试。
    *   **解决方案**:
        *   **具体异常类型**: 捕获更具体的异常，例如 `FeignException`, `IOException`, `BusinessException`。
        *   **统一异常处理器**: 使用 `@RestControllerAdvice` 创建全局异常处理器，对不同类型的异常返回标准化的错误响应。
        *   **细化降级逻辑**: 根据异常类型决定是重试、快速失败还是返回兜底数据。

#### 🔧 中优先级债务

3.  **监控体系有待完善**
    *   **问题**: 项目目前主要依赖日志进行问题排查，缺少体系化的应用性能监控（APM）。核心业务流程（如消息发送的端到端延迟、消息队列的积压情况、缓存命中率）缺乏量化指标。
    *   **影响**:
        *   **被动响应**: 只能在问题发生后通过日志进行分析，无法主动发现潜在的性能瓶颈和系统风险。
        *   **性能评估困难**: 无法精确评估系统在不同负载下的表现，容量规划缺乏数据支撑。
    *   **解决方案**:
        *   **引入Micrometer**: 集成Micrometer，它是Spring Boot Actuator的一部分，可以轻松地将应用指标暴露出来。
        *   **核心指标埋点**:
            *   **API层面**: 使用AOP或拦截器记录每个HTTP接口的QPS、延迟和成功率。
            *   **消息处理**: 监控`im-message-server`处理消息的端到端延迟、各个处理步骤的耗时。
            *   **资源池**: 监控数据库连接池、Redis连接池和Netty线程池的活跃数和等待队列长度。
        *   **可视化**: 配合Prometheus进行数据收集和Grafana进行数据可视化，构建监控仪表盘。

4.  **分布式事务的一致性风险**
    *   **问题**: `PrivateMessageProcessor` 和 `GroupMessageProcessor` 中的消息处理流程涉及多个数据库表的写入（`message`, `user_msg_list` 等）和对Redis的操作。这些操作目前仅由Spring的本地事务保证，并非分布式事务。
    *   **影响**: 在极端情况下（例如，数据库写入成功但后续缓存操作失败），可能导致数据不一致。
    *   **解决方案**:
        *   **评估引入Seata**: 对于需要强一致性的场景，可以考虑引入Seata框架，使用AT模式来管理分布式事务。
        *   **基于消息的最终一致性**: 重新审视业务流程，对于可以接受最终一致性的场景，利用RocketMQ的事务消息功能来确保"数据库操作"和"后续MQ投递"的原子性，下游服务通过幂等消费来保证最终一致性。

#### ⚠️ 低优先级债务

5.  **单元测试与集成测试缺失**
    *   **问题**: 项目缺乏自动化测试用例，尤其是针对核心业务逻辑的单元测试和服务间的集成测试。
    *   **影响**:
        *   **重构困难**: 对现有代码进行优化或重构时，没有测试用例保障，容易引入新的bug。
        *   **代码质量风险**: 无法保证代码变更的正确性，依赖手动测试，效率低下且容易遗漏。
    *   **解决方案**:
        *   **单元测试**: 引入JUnit 5和Mockito，为核心的Service和Processor编写单元测试，专注于业务逻辑的正确性。
        *   **集成测试**: 引入Testcontainers框架，可以在测试环境中动态启动所需的中间件（如Redis, MySQL, RocketMQ），进行更真实的集成测试。

### 🎯 性能与架构优化建议

#### 数据库层面
- **连接池优化**: 根据压力测试结果，精细化调整HikariCP连接池的核心参数，如`maximum-pool-size`, `minimum-idle`, `connection-timeout`等。
- **索引优化**: 针对消息拉取、会话列表查询等高频SQL，使用`EXPLAIN`分析其执行计划，确保索引被有效利用。特别注意针对排序和范围查询的复合索引设计。
- **读写分离**: 对于读多写少的场景（如历史消息查询），可以引入数据库中间件（如ShardingSphere-JDBC）或代理，将读流量路由到从库，降低主库压力。

#### 缓存层面
- **缓存预热与穿透防护**:
  - **预热**: 系统启动时，可以异步加载一些热点数据到Redis中，如热门群聊的成员列表，避免冷启动时的缓存穿透。
  - **空值缓存**: 对于查询为空的结果，也进行短暂缓存（例如1分钟），防止缓存穿透攻击。
- **Redis数据结构优化**: 审视现有的Redis使用方式。例如，对于大量的Hash字段，如果内存占用过高，可以考虑使用Protobuf或JSON序列化后存储在单个String键中，以利用Redis的ziplist或listpack进行内存优化。

#### 架构演进层面
- **API版本管理**: 在Controller的URL中引入版本号（如`/api/v1/...`），使用户端可以平滑地从旧版本迁移到新版本，提高系统的向后兼容性。
- **服务容错与隔离**: 引入Sentinel或Resilience4J等熔断降级组件。为跨服务调用（Feign Client）配置精细的熔断、限流和舱壁隔离策略，防止单个服务的故障引发雪崩效应。
- **分布式追踪**: 集成SkyWalking或Zipkin等分布式追踪系统，将trace ID贯穿整个调用链（从网关到业务服务，再到消息队列），实现请求的可视化追踪，极大简化分布式环境下的问题定位。
---
...
| 笔记本 | Windows | `192.168.101.50` | 部署冗余实例用于测试 |
| Linux 虚拟机 | Linux | `192.168.101.43` | 运行基础设施服务 |

### 2. 服务部署详情

#### 基础架构服务 (Linux 虚拟机 - `192.168.101.43`)
...
| | 笔记本 (`192.168.101.50`) | 1 | 8080 |
| **im-message-server** | 台式机 (`192.168.10.2`) | 1 | 8100 |
| | 笔记本 (`192.168.101.50`) | 1 | 8100 |
