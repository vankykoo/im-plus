...
public class UdpServerHandler extends SimpleChannelInboundHandler<ChatMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ChatMessage msg) throws Exception {
        ...
        System.out.println("[UDP] 收到消息内容: " + processedMsg.getContent() + "，完整消息: " + processedMsg + "，全局消息ID: " + globalMsgId);

        if (ReceiveUserId.SYSTEM_ID.equals(processedMsg.getToId())) {
            logger.info("[UDP] 收到【系统】消息:  发送方ID:{}, 消息类型:{}", processedMsg.getFromId(), ClientToServerMessageType.getLabelByValue(processedMsg.getType()));

            // 处理系统消息，根据消息类型进行分发
            if (processedMsg.getType() == ClientToServerMessageType.LOGIN_REQUEST.getValue()) {
                logger.info("[UDP] 处理登录请求消息");
                // 登录请求处理逻辑
                // 从消息中获取用户ID和token
                ...
                if (token == null || token.isEmpty()) {
                    logger.warn("[UDP] 用户 {} 登录失败: token为空", userId);
                    ctx.channel().close();
                    return;
                }
                ...
                //    logger.warn("[UDP] 用户 {} 登录失败: token无效", userId);
                //    ctx.channel().close();
                //    return;
                //}
                
                logger.info("[UDP] 用户 {} token验证成功", userId);
                ...
                logger.info("[UDP] 处理心跳消息，来自: {}", processedMsg.getFromId());
                // 心跳消息处理逻辑
                // 可以选择回复心跳响应包
                ...
                logger.warn("[UDP] 未知的系统消息类型: {}", processedMsg.getType());
            }
        } else {
            logger.info("[UDP] 收到 [C2C] 消息:  from:{}, to:{}, type:{}", processedMsg.getFromId(), processedMsg.getToId(), ClientToClientMessageType.getLabelByValue(processedMsg.getType()));
